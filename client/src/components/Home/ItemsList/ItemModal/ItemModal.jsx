...
import { Modal, Card, Divider, Col, Row, Typography, Button, Popconfirm } from 'antd';
...
const ItemModal = ({item, visible, onCancel}) => {
  ...
  const handleDataKeyChange = (newKeyValue, id) => {
    const itemIndex = itemData.findIndex(item => item._id === id);
  ...
  return (
    ...
    >
      ...
      <Paragraph editable={{ onChange: setDescription, autoSize: { maxRows: 5, minRows: 3 }}} >{description}</Paragraph>
      <div>
        <Title
          level={3}
          ...
          return (
            <Col span={12} key={data._id}>
              ...
              <Card>
                ...
                <Paragraph
                  editable={{ onChange: (newKeyValue) => handleDataKeyChange(newKeyValue, data._id) }}>{data.key}</Paragraph>
                <Divider/>
                <Title level={5}>Valeur:</Title>
                <Paragraph editable={{ onChange: (newValue) => handleDataValueChange(newValue, data._id) }}>{data.value}</Paragraph>
              </Card>
            </Col>
          )
        })}
